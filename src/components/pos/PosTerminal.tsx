    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          ...
          <div className="rounded-2xl bg-slate-50 dark:bg-zinc-800/50 px-4 py-3 text-sm text-slate-600 dark:text-zinc-400">
            Prefix: <span className="font-semibold text-slate-900 dark:text-zinc-100">{invoicePrefix}</span>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
          <input
            ...
            className="h-11 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-4 text-sm"
          />
          <button
            type="button"
            onClick={() => productsQuery.refetch()}
            className="h-11 rounded-xl border border-slate-300 dark:border-zinc-600 px-5 text-sm font-medium text-slate-900 dark:text-zinc-100 transition hover:bg-slate-50 dark:hover:bg-zinc-800"
          >
            Refresh
          </button>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        {/* Products Section */}
      <section className="rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-zinc-100 mb-4">Products</h2>
          {productsQuery.isLoading ? (
            <p className="text-sm text-slate-500 dark:text-zinc-400">Loading products...</p>
            ...
            <div className="max-h-125 space-y-2 overflow-y-auto pr-2">
              {productsQuery.data?.items.map((product) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-3 rounded-xl border border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800/50 p-3 text-left transition hover:border-slate-300 dark:hover:border-zinc-600 hover:bg-white dark:hover:bg-zinc-900"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-slate-900 dark:text-zinc-100 truncate">{product.name}</div>
              ...
              {productsQuery.data?.items.length === 0 && (
                <p className="text-sm text-slate-500 dark:text-zinc-400">No products found.</p>
              )}
            </div>
          )}
        </section>
        ...
        <aside className="flex flex-col gap-4 rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm max-h-[calc(100vh-200px)] overflow-y-auto">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-zinc-400">
              Checkout
            </p>
            <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900 dark:text-zinc-100">
              Customer details
            </h2>
          </div>
          ...
          <div className="grid gap-3">
            ...
            <textarea
              ...
              className="min-h-20 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-4 py-3 text-sm"
            />
            <input
              value={buyerState}
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-zinc-700 p-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-900 dark:text-zinc-100">Cart</h3>
              <span className="text-xs text-slate-500 dark:text-zinc-400">{items.length} item(s)</span>
            </div>

            <div className="mt-3 max-h-50 space-y-2 overflow-y-auto">
              {items.length === 0 ? (
                <p className="text-sm text-slate-500 dark:text-zinc-400">No items added yet.</p>
              ) : (
                items.map((item, index) => (
                  <div key={item.productId} className="rounded-lg border border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800/50 p-3">
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <div>
                        <div className="text-sm font-medium text-slate-900 dark:text-zinc-100">{item.description}</div>
                        <div className="text-xs text-slate-500 dark:text-zinc-400">SKU {item.sku}</div>
                      </div>
                      <button
                        type="button"
                        onClick={() => removeItem(index)}
                        className="text-xs font-semibold text-red-600 dark:text-red-400 whitespace-nowrap ml-2"
                      >
                        Remove
                      </button>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <label className="block text-xs font-medium text-slate-700 dark:text-zinc-300 mb-1">Qty</label>
                        <input
                          type="number"
                          min="1"
                          value={item.quantity}
                          ...
                          className="w-full h-10 rounded-lg border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-2 py-2 text-sm text-center"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-slate-700 dark:text-zinc-300 mb-1">Price</label>
                        <input
                          type="number"
                          min="0"
                          step="0.01"
                      ...
                      <div>
                        <label className="block text-xs font-medium text-slate-700 dark:text-zinc-300 mb-1">Total</label>
                        <div className="h-10 rounded-lg bg-white dark:bg-zinc-800 border border-slate-300 dark:border-zinc-600 flex items-center justify-center">
          ...
          <div className="rounded-2xl bg-slate-900 p-4 text-white">
            <div className="flex items-center justify-between text-xs text-slate-300 dark:text-zinc-400">
              <span>Subtotal</span>
              <span>{currency.format(totals?.totals.subtotal ?? 0)}</span>
            </div>
            <div className="mt-2 flex items-center justify-between text-xs text-slate-300 dark:text-zinc-400">
              <span>Tax</span>
          ...
          {message && (
            <div className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-xs text-red-700 dark:text-red-400">
              {message}
            </div>
          )}
          ...
          <button
            ...
            onClick={submitSale}
            disabled={submitting}
            className="h-11 py-3 rounded-full bg-slate-900 dark:bg-zinc-100 text-sm font-semibold text-white dark:text-zinc-900 transition hover:bg-slate-700 dark:hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-60"
          >
