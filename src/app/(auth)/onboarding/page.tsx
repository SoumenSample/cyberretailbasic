...
export default function OnboardingPage() {
  ...
  const onSubmit = async (values: BusinessForm) => {
    ...
    if (response.ok) {
      setCreatedBusinessId((data as any)?.businessId ?? null);
      // Refresh the session so the JWT picks up the new businessId and role
      await update();
      setShowSuccess(true);
      return;
    }
      ...
      >
        ...
        {showSuccess && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="max-w-xl rounded-lg bg-white p-6 shadow-lg dark:bg-zinc-900">
              <h3 className="text-lg font-semibold">Business created</h3>
              <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">Your business was created successfully.</p>
