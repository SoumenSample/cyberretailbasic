  ...
  return (
    ...
    <div className="screen-only mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-10">
      <header className="flex flex-col gap-2">
        ...
        <h1 className="text-4xl font-semibold tracking-tight text-slate-900 dark:text-zinc-100">Products</h1>
      ...
      {lowStockView && (
        <section className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4 text-amber-900 dark:text-amber-200 shadow-sm">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <a
              href="/products"
              className="inline-flex h-10 items-center justify-center rounded-xl border border-amber-300 dark:border-amber-700 bg-white dark:bg-zinc-900 px-4 text-sm font-medium text-amber-900 dark:text-amber-200 transition hover:bg-amber-100 dark:hover:bg-amber-800/30"
            >
              Show all products
            </a>
          </div>
      ...
      <section className="grid gap-4 rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-zinc-100">Add product</h2>
        <form className="grid gap-4" onSubmit={onCreateProduct}>
          <div className="grid gap-4 md:grid-cols-3">
            <Field label="Product name">
              <input
                ...
                className="h-11 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 text-sm"
                required
              />
            </Field>
            <Field label="SKU">
            ...
            <Field label="Barcode">
              <div className="flex gap-2">
                <input
                  ...
                  className="h-11 flex-1 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 text-sm"
                />
                <button
                  type="button"
                  onClick={() => setNewBarcode(generateBarcodeValue())}
                  className="h-11 rounded-xl border border-slate-300 dark:border-zinc-600 px-4 text-xs font-medium text-slate-700 dark:text-zinc-300 transition hover:bg-slate-50 dark:hover:bg-zinc-800"
                >
                  Generate
                </button>
              </div>
          ...
          {normalizeBarcode(newBarcode) ? (
            <div className="w-fit rounded-xl border border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800/50 p-3">
      ...
      <section className="grid gap-4 rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-zinc-100">Add category</h2>
        <div className="grid gap-4 md:grid-cols-4">
          <input
            value={categoryName}
      ...
      <section className="grid gap-4 rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-zinc-100">Inventory list</h2>
          <div className="flex flex-wrap gap-3">
            ...
            <button
              type="button"
              className="h-10 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 text-sm font-medium text-slate-700 dark:text-zinc-300 transition hover:bg-slate-50 dark:hover:bg-zinc-800"
              onClick={printLabels}
          ...
          <div className="space-y-2">
            {productsQuery.data?.items.map((item) => {
              ...
              return (
                <div
                  key={item._id}
                   className="flex flex-col gap-3 rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-4 transition hover:bg-slate-50 dark:hover:bg-zinc-800 md:flex-row md:items-center md:justify-between"
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1">
                    ...
                    {normalizeBarcode(item.barcode) ? (
                      <div className="mt-2 w-fit rounded border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-1">
                        <Barcode value={normalizeBarcode(item.barcode)} height={28} width={1.2} displayValue={false} margin={0} />
                      </div>
                    ) : (
                      <div className="text-xs text-amber-700 dark:text-amber-400">No barcode assigned</div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-6">
                    <div>
                      ...
                      <span className="text-slate-900 dark:text-zinc-100">{category ? category.name : "-"}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-medium text-slate-900 dark:text-zinc-100">₹{item.sellingPrice}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-medium text-slate-500">Current stock</span>
                      <span className={`font-medium ${isLowStock ? "text-red-600 dark:text-red-400" : "text-slate-900 dark:text-zinc-100"}`}>
                        {currentStock}
                      </span>
                    </div>
                    <div>
                      <span className="block text-xs font-medium text-slate-500">Min</span>
                      <span className="text-slate-900 dark:text-zinc-100">{item.minStock}</span>
                    </div>
                  </div>

                  <div className="flex gap-2 md:flex-col">
                    <button
                      className="flex-1 rounded-lg border border-slate-300 dark:border-zinc-600 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-zinc-300 transition hover:bg-slate-100 dark:hover:bg-zinc-700 md:flex-none"
                      onClick={() => {
                        setAdjusting(item);
                        setAdjustQty(0);
                        setAdjustNotes("");
                    ...
                    <button
                      className="flex-1 rounded-lg border border-blue-300 dark:border-blue-700 px-3 py-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 transition hover:bg-blue-50 dark:hover:bg-blue-900/20 md:flex-none"
                      onClick={() => startEdit(item)}
                    >
                      Edit
                    </button>
                    <button
                      className="flex-1 rounded-lg border border-red-300 dark:border-red-700 px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 transition hover:bg-red-50 dark:hover:bg-red-900/20 md:flex-none"
                      onClick={() => {
        ...
        <div className="flex items-center justify-between text-sm text-slate-600 dark:text-zinc-400">
          ...
          <div className="flex gap-2">
            <button
              className="rounded-lg border border-slate-300 dark:border-zinc-600 px-3 py-1.5 text-slate-700 dark:text-zinc-300 transition hover:bg-slate-50 dark:hover:bg-zinc-800 disabled:opacity-50"
