...
function ShelfCodePreview({ value }: { value: string }) {
  ...
  if (!qrSrc) {
    return (
      <div className="flex h-28 w-28 items-center justify-center rounded-2xl border border-dashed border-slate-300 dark:border-zinc-600 bg-slate-50 dark:bg-zinc-800/50 text-center text-xs text-slate-500 dark:text-zinc-400">
        QR
        <br />
        {value}
      </div>
  ...
  return <img src={qrSrc} alt={`QR for ${value}`} className="h-28 w-28 rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-2" />;
}
...
export default function ShelvesPage() {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(15,23,42,0.08),transparent_35%),linear-gradient(180deg,#f8fafc_0%,#eef2ff_100%)] dark:bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.05),transparent_35%),linear-gradient(180deg,#09090b_0%,#18181b_100%)]">
      <SiteHeader />
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="min-h-0 flex-1 overflow-auto px-4 py-5 lg:px-6">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
            <section className="rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white/90 dark:bg-zinc-900/90 p-6 shadow-lg shadow-slate-200/60 dark:shadow-zinc-800/60 backdrop-blur">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="space-y-2">
                  ...
                  <h1 className="text-3xl font-semibold tracking-tight text-slate-950 dark:text-zinc-50">Shelf management</h1>
            ...
            <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
              <section className="rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-semibold text-slate-950 dark:text-zinc-50">Create location</h2>
                    <p className="text-sm text-slate-500 dark:text-zinc-400">Add aisles, racks, shelves, or bins with capacity and scan codes.</p>
                  </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Field label="Type">
                    <select
                      ...
                      className="h-10 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 text-sm"
                    >
                  ...
                  <Field label="Parent location">
                    <select value={createForm.parentShelfId} onChange={(event) => setCreateForm((current) => ({ ...current, parentShelfId: event.target.value }))} className="h-10 rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 text-sm">
                ...
                <Field label="Notes">
                  <textarea value={createForm.notes} onChange={(event) => setCreateForm((current) => ({ ...current, notes: event.target.value }))} rows={3} className="w-full rounded-xl border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 py-2 text-sm" placeholder="Optional operational notes" />
                </Field>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  ...
                  {createShelfMutation.isError ? (
                    <p className="self-center text-sm text-red-600 dark:text-red-400">{(createShelfMutation.error as Error).message}</p>
                  ) : null}
                </div>
              </section>

              <section className="rounded-3xl border border-zinc-700 bg-zinc-900 p-6 text-white shadow-xl shadow-zinc-900/20">
                <div className="mb-5">
                ...
                <div className="space-y-5">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <Field label="Shelf">
                        <select value={mapForm.shelfId} onChange={(event) => setMapForm((current) => ({ ...current, shelfId: event.target.value }))} className="h-10 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-white">
                          {shelves.map((shelf) => (
                            <option key={shelf._id} value={shelf._id}>{shelf.code} · {shelf.label}</option>
                          ))}
                        </select>
                      ...
                      <Field label="Product">
                        <select value={mapForm.productId} onChange={(event) => setMapForm((current) => ({ ...current, productId: event.target.value }))} className="h-10 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-white">
                      ...
                      <Field label="Quantity">
                        <Input type="number" min={1} value={mapForm.quantity} onChange={(event) => setMapForm((current) => ({ ...current, quantity: event.target.value }))} className="border-white/10 bg-zinc-900 text-white" />
                      </Field>
                      <Field label="Expiry date">
                        <Input type="date" value={mapForm.expiryDate} onChange={(event) => setMapForm((current) => ({ ...current, expiryDate: event.target.value }))} className="border-white/10 bg-zinc-900 text-white" />
                      </Field>
                      <Field label="Batch">
                        <Input value={mapForm.batchNo} onChange={(event) => setMapForm((current) => ({ ...current, batchNo: event.target.value }))} className="border-white/10 bg-zinc-900 text-white" />
                      </Field>
                    </div>
                    <Field label="Notes">
                      <textarea value={mapForm.notes} onChange={(event) => setMapForm((current) => ({ ...current, notes: event.target.value }))} rows={2} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-white" />
                    </Field>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <Field label="From shelf">
                        <select value={transferForm.fromShelfId} onChange={(event) => setTransferForm((current) => ({ ...current, fromShelfId: event.target.value }))} className="h-10 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-white">
                          {shelves.map((shelf) => (
                            <option key={shelf._id} value={shelf._id}>{shelf.code} · {shelf.label}</option>
                          ))}
                        </select>
                      ...
                      <Field label="To shelf">
                        <select value={transferForm.toShelfId} onChange={(event) => setTransferForm((current) => ({ ...current, toShelfId: event.target.value }))} className="h-10 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-white">
                      ...
                      <Field label="Product">
                        <select value={transferForm.productId} onChange={(event) => setTransferForm((current) => ({ ...current, productId: event.target.value }))} className="h-10 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-white">
                          <option value="">Choose product</option>
                          {products.map((product) => (
                            <option key={product._id} value={product._id}>{product.sku} · {product.name}</option>
                          ))}
                      ...
                      <Field label="Quantity">
                        <Input type="number" min={1} value={transferForm.quantity} onChange={(event) => setTransferForm((current) => ({ ...current, quantity: event.target.value }))} className="border-white/10 bg-zinc-900 text-white" />
                      </Field>
                      <Field label="Expiry date">
                        <Input type="date" value={transferForm.expiryDate} onChange={(event) => setTransferForm((current) => ({ ...current, expiryDate: event.target.value }))} className="border-white/10 bg-zinc-900 text-white" />
                      </Field>
                      <Field label="Batch">
                        <Input value={transferForm.batchNo} onChange={(event) => setTransferForm((current) => ({ ...current, batchNo: event.target.value }))} className="border-white/10 bg-zinc-900 text-white" />
                      </Field>
                    </div>
                    <Field label="Notes">
                      <textarea value={transferForm.notes} onChange={(event) => setTransferForm((current) => ({ ...current, notes: event.target.value }))} rows={2} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-white" />
                    </Field>
                    <div className="mt-3 flex items-center gap-3">
            ...
            <section className="rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-slate-950 dark:text-zinc-50">Shelf directory</h2>
                  <p className="text-sm text-slate-500 dark:text-zinc-400">Scan a shelf code, search by shelf or product (name, SKU, batch), and inspect mapped products with expiry data.</p>
                </div>
              ...
              {shelvesQuery.isLoading ? (
                <p className="mt-4 text-sm text-slate-500 dark:text-zinc-400">Loading shelves...</p>
              ) : shelves.length === 0 ? (
                <p className="mt-4 text-sm text-slate-500 dark:text-zinc-400">No shelves created yet.</p>
              ) : (
                <div className="mt-5 grid gap-4 xl:grid-cols-2">
                  {shelves.map((shelf) => (
                    <article key={shelf._id} className={`rounded-2xl border p-5 shadow-sm ${selectedShelfId === shelf._id ? "border-slate-900 dark:border-zinc-100 bg-slate-50 dark:bg-zinc-800/50" : "border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900"}`} onClick={() => setSelectedShelfId(shelf._id)}>
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <h3 className="text-lg font-semibold text-slate-950 dark:text-zinc-50">{shelf.label}</h3>
                            <Badge variant="outline">{shelf.locationType}</Badge>
                            {shelf.isHighOccupancy ? <Badge className="bg-amber-100 dark:bg-amber-900/30 text-amber-900 dark:text-amber-200">Near capacity</Badge> : null}
                          </div>
                          <p className="mt-1 text-sm text-slate-500 dark:text-zinc-400">Code {shelf.code} · Capacity {shelf.capacityQty || "-"} · Occupancy {formatPct(shelf.occupancyPct)}</p>
                        </div>
                      ...
                      <div className="mt-4 grid gap-4 lg:grid-cols-[auto_1fr]">
                        ...
                        <div className="space-y-3">
                          <div className="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-zinc-700">
                            <div className="h-full rounded-full bg-slate-900 dark:bg-zinc-100" style={{ width: `${Math.min(100, shelf.occupancyPct ?? 0)}%` }} />
                          </div>
                          <div className="grid gap-2 text-sm text-slate-600 dark:text-zinc-400 md:grid-cols-3">
                            <div className="rounded-xl bg-slate-50 dark:bg-zinc-800/50 px-3 py-2">
                              <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-zinc-400">Total qty</div>
                              <div className="font-semibold text-slate-950 dark:text-zinc-50">{shelf.totalQuantity}</div>
                            </div>
                            <div className="rounded-xl bg-slate-50 dark:bg-zinc-800/50 px-3 py-2">
                              <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-zinc-400">Alert threshold</div>
                              <div className="font-semibold text-slate-950 dark:text-zinc-50">{shelf.minOccupancyPct}%</div>
                            </div>
                            <div className="rounded-xl bg-slate-50 dark:bg-zinc-800/50 px-3 py-2">
                              <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-zinc-400">Parent</div>
                              <div className="font-semibold text-slate-950 dark:text-zinc-50">{shelf.parentShelfId ?? "None"}</div>
                            </div>
                          </div>
                            <div className="rounded-2xl bg-slate-50 dark:bg-zinc-800/50 p-3">
                            <div className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-zinc-400">Mapped products</div>
                            {shelf.productsOnShelf.length === 0 ? (
                              <p className="mt-2 text-sm text-slate-500 dark:text-zinc-400">No product mapped to this shelf.</p>
                            ) : (
                              <div className="mt-2 space-y-2">
                                {shelf.productsOnShelf.map((item) => (
                                  <div key={`${item.productId}-${item.expiryDate ?? item.batchNo ?? "na"}`} className="flex flex-col gap-1 rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm md:flex-row md:items-center md:justify-between">
                                    <div>
                                      <div className="font-medium text-slate-900 dark:text-zinc-100">{item.productName}</div>
                                      <div className="text-xs text-slate-500 dark:text-zinc-400">SKU {item.sku} · Batch {item.batchNo ?? "-"} · Expiry {item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : "-"}</div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Button
                                        type="button"
                                        variant="ghost"
                                        className="h-8 px-2 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-700 dark:hover:text-red-300"
                                        onClick={(event) => {
            ...
            <section className="grid gap-6 xl:grid-cols-3">
              ...
              <AlertPanel title="Expiring soon" items={alerts?.expiringSoon ?? []} emptyText="No expiring shelf lots right now." renderItem={(item) => (
                <>
                  <div className="font-medium text-slate-900 dark:text-zinc-100">Shelf {item.shelfCode}</div>
                  <div className="text-xs text-slate-500 dark:text-zinc-400">Qty {item.quantity} · Expiry {new Date(item.expiryDate).toLocaleDateString()}</div>
                </>
              )} />
              <AlertPanel title="Occupancy" items={alerts?.occupancyAlerts ?? []} emptyText="No shelves near capacity." renderItem={(item) => (
                <>
                  <div className="font-medium text-slate-900 dark:text-zinc-100">{item.code} · {item.label}</div>
                  <div className="text-xs text-slate-500 dark:text-zinc-400">{item.totalQuantity}/{item.capacityQty || "-"} ({formatPct(item.occupancyPct)})</div>
                </>
              )} />
            </section>

            {selectedShelf ? (
              <section className="rounded-3xl border border-zinc-700 bg-zinc-900 p-6 text-white shadow-xl">
...
function AlertPanel({
  ...
  return (
    <section className="rounded-3xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
      <h3 className="text-lg font-semibold text-slate-950 dark:text-zinc-50">{title}</h3>
      <div className="mt-4 space-y-3">
        {items.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-zinc-400">{emptyText}</p>
        ) : (
          items.map((item, index) => (
            <div key={index} className="rounded-2xl border border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800/50 px-4 py-3 text-sm">
              {renderItem ? renderItem(item) : (
                <>
                  <div className="font-medium text-slate-900 dark:text-zinc-100">{(item as any).name}</div>
                  <div className="text-xs text-slate-500 dark:text-zinc-400">SKU {(item as any).sku} · Stock {(item as any).stockQty} / Min {(item as any).minStock}</div>
                </>
              )}
            </div>
          ))
