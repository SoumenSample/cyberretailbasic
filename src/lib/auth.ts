  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials, request) {
        ...
        // Find the first membership for this user
  ...
  callbacks: {
    ...
    async jwt({ token, user, ...rest }: any) {
      if (user) {
        ...
        token.globalRole = user.globalRole ?? "ADMIN";
        token.rememberMe = Boolean(user.rememberMe);
      }

      // Fetch business membership for users without a businessId
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.globalRole = token.globalRole as "ADMIN" | "USER";
      }
      return session;
    },
  },
